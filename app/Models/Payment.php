...
class Payment extends Model
{
    ...
    {
        $sql = "INSERT INTO payments (order_id, amount, reference) VALUES (:order_id, :amount, :reference) RETURNING id";
        $stmt = $this->db->prepare($sql);
        $stmt->execute(['order_id' => $orderId, 'amount' => $amount, 'reference' => $reference]);
        if ($stmt) {
            $result = $stmt->fetch();
